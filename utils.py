...
class DataHelper:
    def __init__(
        self, df, window_size, n_features, n_jobs=0, save_folder=None
    ):
        self.df = df
        ...
        if self.save_folder is None:
            self.save_folder = os.path.dirname('data/cross_val')
    ...
    def _save_data(self):
        ...
        labels_filename = os.path.join(
            self.save_folder, f"labels_{self.window_size}_{self.n_features}.csv"
        )
